...
/**
 * 覆盖 valaxy-theme-yun 的 YunProjectCard
 *
 * 原组件在项目未填写 github 字段时，会兜底生成
 * `https://github.com/YunYouJun/${project.name}`（指向原作者的仓库，必然 404）。
 * 此处将兜底地址改为本站作者的 GitHub 主页。
 */
...
// 与 valaxy-theme-yun/client/constants 中的 cubicBezier.easeIn 保持一致
const easeIn: [number, number, number, number] = [0.18, 0.66, 0.05, 0.96]
...
const githubUrl = computed(() => {
  if (props.project.github)
    ...
    return 'https://github.com/wusi321'
})
