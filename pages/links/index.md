---
title: 友情链接
layout: projects
projects:
  blogs:
    title: 个人博客
    emoji: 📝
    collection:
      - name: 半杯凉白开
        emoji: ☁️
        color: '#4682b4'
        desc: 基于 Hexo 的个人博客，记录学习与生活
        textColor: white
        url: https://wusi321.github.io
      - name: SCuNF
        emoji: 📖
        color: '#0f3e69ff'
        desc: 基于 Valaxy 的个人博客，记录学习与生活
        textColor: white
        url: https://scunf.github.io

  ai:
    title: AI 对话
    emoji: 🤖
    collection:
      - name: ChatGPT
        emoji: 💬
        color: '#10a37f'
        desc: OpenAI 推出的对话式 AI 助手
        textColor: white
        url: https://chat.openai.com
      - name: Gemini
        emoji: 🌟
        color: '#4285f4'
        desc: Google 推出的多模态 AI 模型
        textColor: white
        url: https://gemini.google.com
      - name: Grok
        emoji: 🚀
        color: '#1a1a1a'
        desc: xAI 推出的 AI 助手
        textColor: white
        url: https://x.ai
      - name: DeepSeek
        emoji: 🔍
        color: '#4d6bfe'
        desc: 深度求索推出的国产大模型
        textColor: white
        url: https://chat.deepseek.com
      - name: 小米 MiMo
        emoji: 📱
        color: '#ff6900'
        desc: 小米推出的通用智能基座大模型
        textColor: white
        url: https://mimo.xiaomi.com
      - name: Kimi
        emoji: 🌙
        color: '#6c5ce7'
        desc: 月之暗面推出的智能助手
        textColor: white
        url: https://kimi.moonshot.cn
      - name: kimi ppt助手
        emoji: 🔍
        color: '#7c3aed'
        desc: 月之暗面推出的 ppt 助手
        textColor: white
        url: https://www.kimi.com/zh/slides
      - name: 豆包
        emoji: 🫘
        color: '#3370ff'
        desc: 字节跳动推出的 AI 对话助手
        textColor: white
        url: https://www.doubao.com
      - name: 扣子
        emoji: 🧩
        color: '#6b5ce7'
        desc: 字节跳动推出的 AI Bot 开发平台
        textColor: white
        url: https://www.coze.cn/
      - name: 通义千问
        emoji: 🔍
        color: '#ff6b00'
        desc: 阿里推出的 AI 对话助手
        textColor: white
        url: https://www.aliyun.com/product/taobao/tongyiqianwen
      - name: minimax
        emoji: 🔍
        color: '#06b6d4'
        desc: minimax AI 对话助手
        textColor: white
        url: https://www.minimax.cn
      - name: ollama
        emoji: 🔍
        color: '#f59e0b'
        desc: ollama 本地 ai 助手
        textColor: white
        url: https://ollama.com/
  ai-agent:
    title: ai Agent
    emoji: 🤖
    collection:
      - name: Manus
        emoji: 🔍
        color: '#6366f1'
        desc: 全球首款通用人工智能
        textColor: white
        url: https://manus.im/
      - name: Openclaw
        emoji: 🔍
        color: '#f97316'
        desc: Openclaw本地ai助手
        textColor: white
        url: https://openclaws.io/zh/
      - name: Hermes
        emoji: 🔍
        color: '#14b8a6'
        desc: Hermes AI 助手
        textColor: white
        url: https://hermes-agent.nousresearch.com/
        github: https://github.com/NousResearch/hermes-agent
  AI 集成式开发环境:
    title: AI 集成式开发环境
    emoji: 🤖
    collection:
      - name: 微软 Copilot
        emoji: 🔍
        color: '#0078d4'
        desc: 微软推出的 AI 助手
        textColor: white
        url: https://copilot.microsoft.com/
      - name: 谷歌 AI Studio
        emoji: 🔍
        color: '#34a853'
        desc: Google 推出的 AI 助手
        textColor: white
        url: https://aistudio.google.com/
      - name: trae_cn
        emoji: 🔍
        color: '#00d4aa'
        desc: 字节跳动推出的 AI 集成式开发环境
        textColor: white
        url: https://www.trae.cn/
      - name: krio
        emoji: 🔍
        color: '#ff9900'
        desc: 亚马逊推出的 AI 集成式开发环境
        textColor: white
        url: https://www.krio.dev/
      - name: Antigravity
        emoji: 🔍
        color: '#5b4ae0'
        desc: google推出的 AI 集成式开发环境
        textColor: white
        url: https://antigravity.google/
      - name: Qoder
        emoji: 🔍
        color: '#ff6a00'
        desc: 阿里推出的 AI 集成式开发环境
        textColor: white
        url: https://www.qoder.cn/
      - name: cursor
        emoji: 🔍
        color: '#6c3cf0'
        desc:  Cursor 推出的 AI 集成式开发环境
        textColor: white
        url: https://www.cursor.com/
      - name: codex
        emoji: 🔍
        color: '#10a37f'
        desc:  OpenAI 推出的 AI 集成式开发环境
        textColor: white
        url: https://chatgpt.com/zh-Hans-CN/codex/
      - name: claude code
        emoji: 🔍
        color: '#d97706'
        desc: claude code AI 开发环境
        textColor: white
        url: https://code.claude.com/docs/zh-CN/quickstart
      - name: copilot
        emoji: 🔍
        color: '#24292e'
        desc: github的 AI 助手
        textColor: white
        url: https://github.com/copilot
  ai开放平台:
    title: ai 开放平台
    emoji: 🤖
    collection:
      - name: deepseek开放平台
        emoji: 🔍
        color: '#4d6bfe'
        desc: 深度求索推出的国产大模型开放平台
        textColor: white
        url: https://platform.deepseek.com/usage
      - name: 火山引擎开放平台
        emoji: 🔍
        color: '#f2622e'
        desc: 火山引擎推出的国产大模型开放平台
        textColor: white
        url: https://www.volcengine.com/
      - name: gpt开放平台
        emoji: 🔍
        color: '#10a37f'
        desc: OpenAI 推出的 AI 对话助手开放平台
        textColor: white
        url: https://platform.openai.com/
      - name: gemini开放平台
        emoji: 🔍
        color: '#4285f4'
        desc: Google 推出的 AI 对话助手开放平台
        textColor: white
        url: https://aistudio.google.com/prompts/new_chat
      - name: 小米mimo开放平台
        emoji: 🔍
        color: '#ff6900'
        desc: 小米推出的通用智能基座大模型开放平台
        textColor: white
        url: https://platform.xiaomimimo.com/docs/zh-CN/welcome
      - name: 华为开发者联盟
        emoji: 🔍
        color: '#cf0a2c'
        desc: 华为开发者联盟
        textColor: white
        url: https://developer.huawei.com/consumer/cn/
      - name:flameapi
        emoji: 🔍
        color: '#0514c2ff'
        desc: 中转站
        textColor: white
        url: https://api.flameapi.cn/dashboard
  academic:
    title: 学术资源
    emoji: 🎓
    collection:
      - name: 中国知网
        emoji: 📚
        color: '#c62828'
        desc: 中国最大的中文学术资源总库
        textColor: white
        url: https://www.cnki.net
      - name: 谷歌学术
        emoji: 🧠
        color: '#4285f4'
        desc: Google 学术搜索，全球学术文献检索
        textColor: white
        url: https://scholar.google.com
      - name: 学信网
        emoji: 🔍
        color: '#1677ff'
        desc: 中国教育信息网，提供学习资源和资讯
        textColor: white
        url: https://www.xuetangx.com
      - name: IEEE Xplore
        emoji: ⚡
        color: '#0066a1'
        desc: 国际电气电子工程师学会学术文献库
        textColor: white
        url: https://ieeexplore.ieee.org
      - name: Science Citation Index Expanded
        emoji: 🔍
        color: '#6366f1'
        desc: 国际电气电子工程师学会学术文献检索系统
        textColor: white
        url: https://www.scimagojr.com
      - name: 艾斯科蓝
        emoji: 🔍
        color: '#0891b2'
        desc: EI会议学术交流系统
        textColor: white
        url: https://www.ais.cn/attendees
      - name: Web of Science
        emoji: 🌐
        color: '#3a1c71'
        desc: 全球权威科学引文索引数据库
        textColor: white
        url: https://www.webofscience.com
      - name: ScienceDirect
        emoji: 📄
        color: '#ff6c2d'
        desc: Elsevier 学术期刊全文数据库
        textColor: white
        url: https://www.sciencedirect.com
      - name: PubScholar
        emoji: 🏛️
        color: '#1a73e8'
        desc: 中科院公益学术平台，免费获取论文
        textColor: white
        url: https://pubscholar.cn
      - name: 万方数据
        emoji: 🔬
        color: '#005a9c'
        desc: 国内知名中文学术文献检索平台
        textColor: white
        url: https://www.wanfangdata.com.cn

  cloud:
    title: 云平台
    emoji: ☁️
    collection:
      - name: 百度飞桨
        emoji: 🚣
        color: '#2932e1'
        desc: 百度深度学习平台 PaddlePaddle
        textColor: white
        url: https://www.paddlepaddle.org.cn
      - name: 阿里云
        emoji: ☁️
        color: '#ff6a00'
        desc: 阿里巴巴旗下云计算服务平台
        textColor: white
        url: https://www.aliyun.com

  tutorials:
    title: 教程资源
    emoji: 📚
    collection:
      - name: GitHub
        emoji: 🐙
        color: '#24292e'
        desc: 全球最大代码托管与协作平台
        textColor: white
        url: https://github.com
      - name: CSDN
        emoji: ✍️
        color: '#fc5531'
        desc: 中国专业 IT 技术社区
        textColor: white
        url: https://www.csdn.net
      - name: 知乎
        emoji: 💡
        color: '#0084ff'
        desc: 中文互联网高质量问答社区
        textColor: white
        url: https://www.zhihu.com
      - name: 百度
        emoji: 🔎
        color: '#2932e1'
        desc: 全球最大中文搜索引擎
        textColor: white
        url: https://www.baidu.com
  ROBOT:
    title: 机器人相关收集--Robocon
    emoji: 🔧 
    collection:
      - name: FishRos
        emoji: 🐟
        color: '#3cb371'
        desc: ROS 机器人开发学习社区
        textColor: white
        url: https://fishros.com
      - name: 犬小哈教程
        emoji: 🐾
        color: '#f59e0b'
        desc: 犬小哈教程网站
        textColor: white
        url: https://www.quanxiaoha.com/
      - name: 立创硬件开源社区
        emoji: 🔧
        color: '#005bac'
        desc: 立创硬件开源社区
        textColor: white
        url: https://oshwhub.com/
      - name: 拓竹3D打印
        emoji: 🔧
        color: '#00b050'
        desc: 拓竹3D打印社区
        textColor: white
        url: https://makerworld.com.cn/zh
      - name: lain
        emoji: 🔧
        color: '#7c3aed'
        desc: lain 的运控
        textColor: white
        url: https://locowiki.github.io/
      - name: 机器人工程师本科学习计划--lain
        emoji: 🔧
        color: '#2563eb'
        desc: 机器人工程师本科学习计划
        textColor: white
        url: https://lain-database.feishu.cn/wiki/PgYswrFOZixDJEkilrPcSJ1cnRD?from=from_copylink
      - name: 机器人体系化教学文档--lain
        emoji: 🔧
        color: '#dc2626'
        desc: 机器人体系化教学文档
        textColor: white
        url: https://robotics-tutorial-wiki.vercel.app
      - name: Robotics Tutorial
        emoji: 🔧
        color: '#0891b2'
        desc: Robotics Tutorial 是一个开源的机器人学习项目
        textColor: white
        url: https://github.com/Michael-Jetson/Robotics_Tutorial
      - name: 云urdf测试平台
        emoji: 🔧
        color: '#9333ea'
        desc: 云urdf测试平台
        textColor: white
        url: https://viewer.robotsfan.com/
      - name: 简历自检--lain
        emoji: 🔧
        color: '#0369a1'
        desc: 简历教程
        textColor: white
        url: https://lain-database.feishu.cn/wiki/YvLewZLUKiKFVzkzUbsc8PCznbe
      - name: 宇树机器开源项目
        emoji: 🔧
        color: '#ea580c'
        desc: 宇树机器开源项目
        textColor: white
        url: https://www.unifolm.com/
        github: https://github.com/unitreerobotics
      - name: RM社区
        emoji: 🔧
        color: '#b91c1c'
        desc: RM社区
        textColor: white
        url: https://bbs.robomaster.com/
  competitions:
    title: 学科竞赛
    emoji: 🏆
    collection:
      - name: 全国大学生电子设计竞赛
        emoji: 🔌
        color: '#d32f2f'
        desc: 教育部主办，电子信息类 A 类学科竞赛
        textColor: white
        url: https://nuedc.xjtu.edu.cn
      - name: 全国大学生智能汽车竞赛
        emoji: 🏎️
        color: '#ff5722'
        desc: 中国电子学会主办，智能汽车设计竞赛
        textColor: white
        url: http://smartcarrace.com/
      - name: 全国大学生嵌入式系统设计竞赛
        emoji: 🔧
        color: '#388e3c'
        desc: 中国电子学会主办，嵌入式系统设计竞赛
        textColor: white
        url: https://www.embedcn.org
      - name: 全国大学生物联网设计竞赛
        emoji: 🌐
        color: '#1976d2'
        desc: 华为等企业联合支持的物联网顶级赛事
        textColor: white
        url: http://iot.sjtu.edu.cn
      - name: 中国软件杯
        emoji: 💻
        color: '#00bcd4'
        desc: 工信部/教育部/江苏省政府联合主办
        textColor: white
        url: https://www.cnsoftbei.com
      - name: Robocon
        emoji: 🤖
        color: '#e64a19'
        desc: 全国大学生机器人大赛，ABU 国际赛
        textColor: white
        url: http://www.cnrobocon.net
      - name: RoboCup
        emoji: ⚽
        color: '#1565c0'
        desc: 机器人世界杯，国际顶尖机器人赛事
        textColor: white
        url: https://www.robocup.org
      - name: RoboMaster
        emoji: 🔫
        color: '#b71c1c'
        desc: 大疆主办的机器人对抗赛事
        textColor: white
        url: https://www.robomaster.com
      - name: 高教杯数学建模
        emoji: 📐
        color: '#4caf50'
        desc: 全国大学生数学建模竞赛 (CUMCM)
        textColor: white
        url: http://www.mcm.edu.cn
      - name: 数学建模美赛
        emoji: 🌍
        color: '#ff9800'
        desc: 美国大学生数学建模竞赛 (MCM/ICM)
        textColor: white
        url: https://www.comap.com
      - name: 统计建模
        emoji: 📊
        color: '#9c27b0'
        desc: 全国大学生统计建模大赛
        textColor: white
        url: http://tjjmds.ai-learning.net
      - name: 全球校园人工智能算法精英大赛
        emoji: 🏅
        color: '#2563eb'
        desc: 全球校园人工智能算法精英大赛
        textColor: white
        url: https://www.aicomp.cn/
      - name: 摩课云
        emoji: 🏅
        color: '#2196f3'
        desc: 高校学生竞赛与教师发展数据服务平台
        textColor: white
        url: https://cc.moocollege.com
      - name: 华为ict程序设计大赛
        emoji: 🔧
        color: '#cf0a2c'
        desc: 华为ict程序设计大赛
        textColor: white
        url: https://e.huawei.com/cn/talent/ict-academy/#/home
      
  education:
    title: 教学平台
    emoji: 🎬
    collection:
      - name: 哔哩哔哩
        emoji: 📺
        color: '#fb7299'
        desc: 国内最大弹幕视频网，海量学习资源
        textColor: white
        url: https://www.bilibili.com
      - name: 中国大学 MOOC
        emoji: 🏫
        color: '#2e8b57'
        desc: 高等教育出版社联合名校的在线课程
        textColor: white
        url: https://www.icourse163.org

  software:
    title: 开发平台
    emoji: 🔧
    collection:
      - name: ST 意法半导体
        emoji: 💎
        color: '#00599c'
        desc: 全球领先的 MCU 半导体制造商
        textColor: white
        url: https://www.st.com
      - name: TI 德州仪器
        emoji: 📟
        color: '#cc0000'
        desc: 全球知名模拟与嵌入式芯片厂商
        textColor: white
        url: https://www.ti.com
      - name: 乐鑫科技
        emoji: 📡
        color: '#e7352c'
        desc: 知名 Wi-Fi/蓝牙 MCU 芯片厂商 (ESP32)
        textColor: white
        url: https://www.espressif.com

  other:
    title: 其他链接
    emoji: 🔗
    collection:
      - name: 软仓
        emoji: 🛠️
        color: '#607d8b'
        desc: 软仓-实用工具与资源导航站
        textColor: white
        url: https://ruancang.net
      - name: 果核剥壳
        emoji: 🔧
        color: '#e11d48'
        desc: 果核剥壳-软件逆向分析工具
        textColor: white
        url: https://www.ghxi.com
      - name: 铨州智造
        emoji: 🔧
        color: '#0284c7'
        desc: 铨州CNC铝合金碳纤维加工
        textColor: white
        url: https://gy2025.com/
      - name: 哈长伟
        emoji: 🔧
        color: '#ca8a04'
        desc: 哈长伟碳纤维加工
        textColor: white
        url: https://www.grouphrc.com/cn
      - name: 嘉立创
        emoji: 🔧
        color: '#7c3aed'
        desc: 嘉立创
        textColor: white
        url: https://www.lceda.cn/

  shopping:
    title: 购物平台
    emoji: 🛒
    collection:
      - name: 淘宝
        emoji: 🛍️
        color: '#ff5000'
        desc: 阿里巴巴旗下 C2C 电商平台
        textColor: white
        url: https://www.taobao.com
      - name: 天猫
        emoji: 🐱
        color: '#ff0036'
        desc: 阿里巴巴旗下 B2C 品牌电商平台
        textColor: white
        url: https://www.tmall.com
      - name: 京东
        emoji: 🐶
        color: '#e2231a'
        desc: 中国领先的综合电商与物流平台
        textColor: white
        url: https://www.jd.com
      - name: 闲鱼
        emoji: 🐟
        color: '#ffc300'
        desc: 阿里巴巴旗下二手交易平台
        textColor: white
        url: https://www.goofish.com
      - name: 立创商城
        emoji: 💾
        color: '#005bac'
        desc: 电子元器件一站式采购平台
        textColor: white
        url: https://www.szlcsc.com
      - name: api中转站
        emoji: 🔧
        color: '#007bff'
        desc: 提供 API 服务的中转站
        textColor: white
        url: https://api.waveapi.top
---

